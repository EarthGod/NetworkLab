...
void ip::push(int port, Packet *packet) {
	...
	if(port == 0){
		...
		struct MyIPHeader *ipheader = (struct PacketHeader *)packet->data();
		if (ipheader->ttl <= 0){
			packet->kill();
			return;
		}
