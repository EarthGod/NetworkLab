...
BasicClient::BasicClient() : _timer(this) {
	_seq = 0;
	_period = 3;
	_delay = 0;
	_time_out = 1;
...
int BasicClient::initialize(ErrorHandler *errh){
	...
	if(_delay>0){
    	_timer.schedule_after_sec(_delay);
	}
    return 0;
}
...
void BasicClient::run_timer(Timer *timer) {
	...
	//READ FILE DATA AND PUT IT HERE
	memcpy(data, "hello", 5);
    output(0).push(packet);
	transmissions++;
    _timer.schedule_after_sec(_time_out);
}
...
void BasicClient::push(int port, Packet *packet) {
	...
	if(port == 0){ //data -> send ack
		...
		if(header->sequence == _seq) {
			...
			_seq++;
			transmissions=0;
			_timer.schedule_after_sec(_period);
